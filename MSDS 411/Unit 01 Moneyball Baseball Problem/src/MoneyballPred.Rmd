...
library(psych)
```
Here we read the data file. Making use of R's `file.choose()` function to make it easier if I need to work from another laptop.
```{r}
...
Let's start with R's summary functions, `str, summary, describe` function
```{r}
str(moneyball)
summary(moneyball)
describe(moneyball)
...
Let's explore each column to understant their distribution

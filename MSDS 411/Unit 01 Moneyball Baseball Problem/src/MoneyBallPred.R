...
moneyball = read.csv(
  '/Users/legs_jorge/Documents/Data Science Projects/MSDS_Northwestern/MSDS 411/Unit 01 Moneyball Baseball Problem/Data/moneyball.csv',
  header = T
)
...
# Let's create a series of scatter plots to understand how each independent
# variable interacts with the dependent variable. These scatter plots will
# help us spot any infrigement of the assupmtions, namely linearity

# Basic Scatterplot Matrix with correlation and fitted line

chart.Correlation(moneyball, histogram = TRUE, pch = 1)

# In the above plot:
#
# The distribution of each variable is shown on the diagonal.
# On the bottom of the diagonal : the bivariate scatter plots
# with a fitted line are displayed on the top of the diagonal : the value of
# the correlation plus the significance level as stars
# Each significance level is associated to a symbol :
# p-values(0, 0.001, 0.01, 0.05, 0.1, 1) <=> symbols(“***”, “**”, “*”, “.”, " “)

# as we go across the second row, we notice that that variables aren't strongly 
# correalated to our target variable.
# One can also notice correlation numbers up to 0.97 among our independent
# variables. Further investigation needed.
# 1.0 - Preprocessing ---------------------------------
...
marginplot(moneyball[, c("TEAM_BATTING_HBP", "TEAM_BASERUN_CS")],
           col = mdc(1:2),
           cex.numbers = 1.2,
           pch = 19)
marginplot(moneyball[, c("TEAM_FIELDING_DP", "TEAM_BASERUN_SB")],
           col = mdc(1:2),
           cex.numbers = 1.2,
           pch = 19)
marginplot(moneyball[, c("TEAM_BATTING_SO", "TEAM_PITCHING_SO")],
           col = mdc(1:2),
           cex.numbers = 1.2,
           pch = 19)
...
xyplot(
  mice_imputes,
  TEAM_BASERUN_SB ~ TEAM_BATTING_SO | .imp,
  pch = 20,
  cex = 1.4
)
...
par(mfrow = c(2, 2))
hist(
  Prep_moneyball$TEAM_BATTING_H,
  col = "#A71930",
  xlab = "Team_Batting_H",
  main = "Histogram of Hits"
)
hist(
  Prep_moneyball$TEAM_BATTING_2B,
  col = "#09ADAD",
  xlab = "Doubles",
  main = "Histogram of Doubles"
)
boxplot(Prep_moneyball$TEAM_BATTING_H,
        col = "#A71930",
        main = "Boxplot of Hits")
boxplot(Prep_moneyball$TEAM_BATTING_2B,
        col = "#09ADAD",
        main = "Boxplot of Doubles")
par(mfrow = c(1, 1))

# Triples and Home Runs
par(mfrow = c(2, 2))
hist(
  Prep_moneyball$TEAM_BATTING_3B,
  col = "#A71930",
  xlab = "Triples",
  main = "Histogram of Triples"
)
hist(
  Prep_moneyball$TEAM_BATTING_HR,
  col = "#DBCEAC",
  xlab = "Home Runs",
  main = "Histogram of Home Runs"
)
boxplot(Prep_moneyball$TEAM_BATTING_3B,
        col = "#A71930",
        main = "Boxplot of Triples")
boxplot(Prep_moneyball$TEAM_BATTING_HR,
        col = "#DBCEAC",
        main = "Boxplot of Home Runs")
par(mfrow = c(1, 1))

# Walks, Strikeouts, HBP
par(mfrow = c(2, 3))
hist(
  Prep_moneyball$TEAM_BATTING_BB,
  col = "#A71930",
  xlab = "Walks",
  main = "Histogram of Walks"
)
hist(
  Prep_moneyball$TEAM_BATTING_SO,
  col = "#09ADAD",
  xlab = "Strikeouts",
  main = "Histogram of Strikeouts"
)
hist(
  Prep_moneyball$TEAM_BATTING_HBP,
  col = "#DBCEAC",
  xlab = "Hit By Pitches",
  main = "Histogram of HBP"
)
boxplot(Prep_moneyball$TEAM_BATTING_BB,
        col = "#A71930",
        main = "Boxplot of Walks")
boxplot(Prep_moneyball$TEAM_BATTING_SO,
        col = "#09ADAD",
        main = "Boxplot of Strikeouts")
boxplot(Prep_moneyball$TEAM_BATTING_HBP,
        col = "#DBCEAC",
        main = "Boxplot of HBP")
par(mfrow = c(1, 1))
...
par(mfrow = c(2, 2))
hist(
  Prep_moneyball$TEAM_BASERUN_SB,
  col = "#A71930",
  xlab = "Stolen Bases",
  main = "Histogram of Steals"
)
hist(
  Prep_moneyball$TEAM_BASERUN_CS,
  col = "#DBCEAC",
  xlab = "Caught Stealing",
  main = "Histogram of CS"
)
boxplot(Prep_moneyball$TEAM_BASERUN_SB,
        col = "#A71930",
        main = "Boxplot of Steals")
boxplot(Prep_moneyball$TEAM_BASERUN_CS,
        col = "#DBCEAC",
        main = "Boxplot of CS")
par(mfrow = c(1, 1))
...
par(mfrow = c(2, 2))
hist(
  Prep_moneyball$TEAM_PITCHING_H,
  col = "#A71930",
  xlab = "Hits Against",
  main = "Histogram of Hits Against"
)
hist(
  Prep_moneyball$TEAM_PITCHING_HR,
  col = "#09ADAD",
  xlab = "Home Runs Against",
  main = "Histograms of HR Against"
)
boxplot(Prep_moneyball$TEAM_PITCHING_H,
        col = "#A71930",
        main = "Boxplot of Hits Against")
boxplot(Prep_moneyball$TEAM_PITCHING_HR,
        col = "#09ADAD",
...
par(mfrow = c(2, 2))
hist(
  Prep_moneyball$TEAM_PITCHING_BB,
  col = "#A71930",
  xlab = "Walks Allowed",
  main = "Histogram of Walks Allowed"
)
hist(
  Prep_moneyball$TEAM_PITCHING_SO,
  col = "#DBCEAC",
  xlab = "Strikeouts",
  main = "Histograms of Strikeouts"
)
boxplot(Prep_moneyball$TEAM_PITCHING_BB,
        col = "#A71930",
        main = "Boxplot of Walks Allowed")
boxplot(Prep_moneyball$TEAM_PITCHING_SO,
        col = "#DBCEAC",
        main = "Boxplot of Strikeouts")
par(mfrow = c(1, 1))
...
par(mfrow = c(2, 2))
hist(
  Prep_moneyball$TEAM_FIELDING_DP,
  col = "#A71930",
  xlab = "Double Plays",
  main = "Histogram of Double Plays"
)
hist(
  Prep_moneyball$TEAM_FIELDING_E,
  col = "#09ADAD",
  xlab = "Errors Committed",
  main = "Histogram of Errors Committed"
)
boxplot(Prep_moneyball$TEAM_FIELDING_DP,
        col = "#A71930",
        main = "Boxplot of Double Plays")
boxplot(Prep_moneyball$TEAM_FIELDING_E,
        col = "#09ADAD",
...
# One way to deal with it is to use Hubers function to not weight all the
# points equally. The outliers should have less weight on the model.
#
...
# Any fields that are a linear combination of each other?
findLinearCombos(Prep_moneyball)
# 3.0 - Model Training ---------------------------------
model1 <- lm(PC2 ~ ., data = Prep_moneyball[, -c(1, 11)])
summary(model1)
